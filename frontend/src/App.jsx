...
function App() {
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      setDoc(data.documentation || '⚠️ No insights returned.');
    } catch (error) {
      toast.error('❌ Failed to generate insights. Please try again.');
      console.error('Error:', error);
      setDoc('⚠️ Error generating insights.');
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const handleCopyDoc = () => {
    ...
    setDocCopyMessage('✅ Insights copied to clipboard!');
  ...
  const handleExportDocTXT = () => {
    const docContent = `
      ===========================
      Insights for ${language.toUpperCase()} Code:
      ===========================

      ${doc}
    `;
    const blob = new Blob([docContent], { type: 'text/plain;charset=utf-8' });
    saveAs(blob, `insights-${language}.txt`);
  };
      ...
      <motion.div className="w-1/2 p-6 flex flex-col gap-4 bg-black/60 backdrop-blur-md shadow-3d rounded-2xl">
        ...
        <div className="flex-1 min-h-0 flex flex-col">
          {isLoading ? (
            ...
            <div className="flex flex-1 items-center justify-center w-full">
              <span className="italic text-gray-350 text-lg opacity-100 select-none">Insights will appear here...</span>
            </div>
          )}
        </div>
